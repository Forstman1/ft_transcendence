...
export class ProfileService {
  ...
  async calculateChartData(
    userId: string,
  ): Promise<activitiesHistoryType | null> {
    try {
      ...
      gameHistory.forEach((entry) => {
        ...
        const monthName = new Date(entry.createdAt).toLocaleString('en-US', {
          month: 'long',
        });
      ...
      const adjustedLabels = labels
        .slice(startMonth)
        .concat(labels.slice(0, startMonth));
      const adjustedMatches = matchesPerMonth
        .slice(startMonth)
        .concat(matchesPerMonth.slice(0, startMonth));
  ...
  async calculateMatchesResults(
    userId: string,
  ): Promise<matchesResultsType | null> {
    try {
      const gameHistory = await this.prismaService.gameHistory.findMany({
        where: { userId: userId },
      });
  ...
  async determineAchievements(
    userId: string,
  ): Promise<achievementsType | null> {
    try {
      const gameHistoryData: GameHistory[] | null =
        await this.prismaService.gameHistory.findMany({
          where: { userId: userId },
        });

      const totalXP = gameHistoryData.reduce(
        (totalXP, entry) => totalXP + entry.xp,
        0,
      );
      const totalMatches = gameHistoryData.length;
      const totalRounds = gameHistoryData.reduce(
        (total, entry) => total + entry.rounds,
        0,
      );
  ...
  async calculateMatchesHistory(
    userId: string,
  ): Promise<matchesHistoryType | []> {
    try {
      const gameHistory = await this.prismaService.gameHistory.findMany({
        where: { userId: userId },
        include: {
